...
export const WORKER_OPERATING_ENVELOPE = [
	...
	'- You MAY make code changes locally and push a NEW work branch (never the default branch, never force-push) so a pull request can be opened from it. But do NOT open, merge, edit, or review pull requests yourself, do not close or comment on issues, and do not call any other GitHub write API. To land a change, surface exactly ONE typed action from the catalog in your emitted result (for example `create_pr` with `autoMerge: true` -- the create-a-PR-and-auto-merge happy path -- or `merge_pr` for an existing PR); the host performs the write under the user\'s identity only after explicit confirmation. The exact set of actions you may surface, with their required payload fields, is listed in the "Action catalog" section of your task brief; choose only from it.',
...
export function buildWorkerBrief(role: WorkerRole, task: ILogicalTask): string {
	...
	const lines: string[] = [
		...
		`You were dispatched with the ${role} lens, but you are not limited to it: assess THIS work item and take the action that is genuinely best for it. If implementing a fix is the right move, do it: make the smallest verified change on a NEW work branch (never the default branch), push that branch, then surface the \`create_pr\` action (set \`autoMerge: true\` for the create-a-PR-and-auto-merge happy path) so the human can approve landing it. For a failing CI check, fix it the same way: change on a work branch, push, then \`create_pr\` (or \`merge_pr\` if a PR already exists). You may equally review, triage/group, or -- when only a human can decide -- surface a question. Use the mounted skill that fits the action you choose. Do not force a triage-only result when the item clearly warrants implementation.`,
		'',
		'## How to proceed',
